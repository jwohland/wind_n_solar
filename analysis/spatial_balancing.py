...
def plot_histograms(wind_shares, solar_shares, plotname):
            ...
            for i, country_group in enumerate(country_generation[data_timescale].keys()):
                ...
                if exclude == country_group:
                    ...
                    amplitude_dict[exclude]["alone"] = np.round(
                        float((G_exclude.max() - G_exclude.min()) / G_exclude.min() * 100),
                        1,
                    )
                else:
    ...
    for i, data_timescale in enumerate(results.keys()):
        ...
        ticklabels = list(results[data_timescale].keys()[1:-1])  # for plotting asthaetics, last group with linebreak
        ticklabels.append("Greece, Bulgaria, Serbia, Croatia \n Bosnia and Herzogovina, Romania, Albania")
        ax.set_xticklabels(
            ticklabels,
            rotation=45,
            ha="right",
        ...
        ax.axhline(
            results[data_timescale]["All"]["others"], color="red", ls="--", label="Europe"
        )
        ax.set_ylabel(
            r"Amplitude of variability   $\frac{G_\mathrm{max} -G_\mathrm{min}}{G_\mathrm{min}}$ [%]", fontsize=fs
        )
...
"""
SCENARIO1: current installed capacities
"""
wind_shares = [
    8.6,
    9.4,
    ...
    1.0,
    2.5,
]  #  Country wind capacity divided by European wind + solar capacity
solar_shares = [
    4.1,
    3.6,
    6.7,
    ...
    0.1,
    1.6,
]  #  Country solar capacity divided by European wind + solar capacity
