...
for time_scale in ['annual', 'ten', 'twenty'][1:]:
    ...
    for turbine_name in turbine_names:
        ...
        for number in all_power.number.values:
            ...
            for i in range(5):
                vmax = np.round(np.max([eofs.values.max(), -eofs.values.min()])*1.1, 2)
                if i != 0:
                    ...
                    plot_field(eofs.sel({'mode': i}, drop=True), ax=ax[0, i],
                           title= str(np.round(variance_fraction.sel({'mode': i}).values*100, 1)) + '% variance ',
                           vmin=-vmax, vmax=vmax, add_colorbar=True, cbar_ax=cbar_ax,
                           cmap=cmap, cbar_kwargs={'orientation': 'horizontal'})
